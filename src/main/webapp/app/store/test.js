	...
	var CLODOP = {
		...
		Printers : {
			...
			"list" : [{
						...
						"name" : "HP LaserJet Pro MFP M128fw",
						"DriverName" : "HP LaserJet Pro MFP M127-M128 PCLmS",
						"PortName" : "WSD-c561e2a6-f5d5-4e04-b969-3a997df721a2.003f",
						"Orientation" : "1",
						"PaperSize" : "9",
						"PaperLength" : "2970",
						"PaperWidth" : "2100",
		...
		FORMAT : function(oType, oValue) {
			...
			if (oType !== undefined && oValue !== undefined) {
				if (oType.replace(/^\s+|\s+$/g, "").toLowerCase()
						.indexOf("time:") == 0) {
					...
					oValue = oValue.replace(/\(.*\)/g, " ");
					oValue = oValue
							.replace(/星期日|星期一|星期二|星期三|星期四|星期五|星期六/g, " ");
						...
						if (oType.toLowerCase() == "weekindex")
							...
							if (/(m+:)/i.test(oValue))
								oValue = oValue.replace(RegExp.$1, ("00"
												...
												- RegExp.$1.length));
							if (/(M+)/i.test(oValue)) {
								var dsWidth = ("" + iMonth).length > RegExp.$1.length
										? ("" + iMonth).length
										: RegExp.$1.length;
							...
							if (/(d+)/i.test(oValue)) {
								var dsWidth = ("" + iDate).length > RegExp.$1.length
										? ("" + iDate).length
										: RegExp.$1.length;
								oValue = oValue.replace(RegExp.$1,
										("00" + iDate)
		...
		NewPage : function() {
			...
			if (blSomeNormal)
				this.defStyleJson["beginpage"] = this.defStyleJson["beginpage"]
						+ 1;
		},
		NEWPAGEA : function() {
			this.NewPageA();
		...
		NewPageA : function() {
			...
			if (blSomeNormal)
				this.defStyleJson["beginpagea"] = this.defStyleJson["beginpagea"]
						+ 1;
		},
		...
		RemoveIframes : function() {
			...
			try {
				for (var i = 0; i < this.Iframes.length; i++) {
					...
					if ((now - this.Iframes[i]["time"]) > this.timeThreshold
							* 60000) {
		...
		SelectBox : {
			...
			create : function(iW, iH, onlySelect) {
				...
				areaDiv.appendChild(CLODOP.creatLabelElement("span", "选打印机：",
						200, 46, 67));
				...
				areaDiv.appendChild(CLODOP.creatLabelElement("span", "打印份数：",
						200, 46, 121));
